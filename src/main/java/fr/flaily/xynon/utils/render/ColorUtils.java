...
public class ColorUtils {
    ...
    public static int astolfo(float seconds, float saturation, float brightness, float index) {
        if (seconds <= 0f) seconds = 1f;                 // guard
        double period = seconds * 1000.0;                // milliseconds loop length
        double time = (System.currentTimeMillis() + (long) index) % period;
        double hue = time / period;                      // 0.0 .. 1.0 (smooth)

        // triangular "Astolfo" wave: 0..0.5 -> 0..0.5, 0.5..1 -> 0.5..0  -> then shift up
        if (hue > 0.5) hue = 1.0 - hue;                  // mirror upper half -> 0..0.5
        hue += 0.5;                                      // shift into 0.5..1.0 range
