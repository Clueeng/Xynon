...
public class RenderUtil {
    ...
    public static ResourceLocation getTextureFromUrl(Minecraft mc, String imageUrl, String imageName) {
        ...
        try {
            ...
            ResourceLocation resourceLocation = mc.getTextureManager()
                    .getDynamicTextureLocation(imageName, dynamicTexture);
    ...
    public static void drawMultiColorGradient(float x, float y, float x1, float y1, int[] colors, boolean vertical) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            if (vertical) {
                // Top color
                glColor(color1);
                GL11.glVertex2f(x, y + t1 * (y1 - y));
                GL11.glVertex2f(x1, y + t1 * (y1 - y));

                // Bottom color
                glColor(color2);
                GL11.glVertex2f(x1, y + t2 * (y1 - y));
                GL11.glVertex2f(x, y + t2 * (y1 - y));
            } else {
                // Left color
                glColor(color1);
                GL11.glVertex2f(x + t1 * (x1 - x), y);
                GL11.glVertex2f(x + t1 * (x1 - x), y1);

                // Right color
                glColor(color2);
                GL11.glVertex2f(x + t2 * (x1 - x), y1);
                GL11.glVertex2f(x + t2 * (x1 - x), y);
            }
        }
    ...
    public static void drawRoundedRect23(float n, float n2, float n3, float n4, final int n5, final int n6) {
        GL11.glDisable(2929);
        GL11.glEnable(3042);
        GL11.glDisable(3553);
        GL11.glBlendFunc(770, 771);
    ...
    public static void drawQuadsShader() {
    ...
    public static void drawHUE(float x, float y, float width, float height) {
        ...
        for(int col : colors) {
            float drawHeight = height / colors.length;
            float left = x;
            float right = x + width;
            float top = y + yOffset;
            ...
            // colors
            float f = (col >> 24 & 255) / 255.0f;
            float f1 = (col >> 16 & 255) / 255.0f;
            float f2 = (col >> 8 & 255) / 255.0f;
            float f3 = (col & 255) / 255.0f;
