...
public final class CustomFontRenderer {
    ...
    public void drawString(final String text, final float x, final float y2, final int color) {
        renderString(getText(text), x, y2, color, false);
    }
    ...
    public void drawStringWithShadow(final String text, final float x, final float y2, final int color) {
        ...
        renderString(getText(text), x, y2, color, true);
        GL11.glTranslated(-0.5, -0.5, 0.0);
        renderString(getText(text), x, y2, color, false);
    ...
    private void renderString(String text, float x, float y2, final int color, final boolean shadow) {
        ...
        final float multiplier = 255.0f * (shadow ? 4 : 1);
        final Color c2 = new Color(color);
        GlStateManager.color(c2.getRed() / multiplier, c2.getGreen() / multiplier, c2.getBlue() / multiplier, (float) ((color >> 24 & 255) / 255.0));
