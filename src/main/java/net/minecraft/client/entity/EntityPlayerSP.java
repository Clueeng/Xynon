...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public float prevServerYaw, prevServerPitch;
    public boolean lastReportedOnGround;
    ...
    {	
        ...
        {
            RotationEvent eventRotation = new RotationEvent(this.rotationYaw, this.rotationPitch, serverYaw, serverPitch);
            Xynon.INSTANCE.getEventBus().post(eventRotation);

            serverYaw = eventRotation.getYaw();
            serverPitch = eventRotation.getPitch();

            prevServerYaw = eventRotation.getPrevYaw();
            prevServerPitch = eventRotation.getPrevPitch();

            float[] gcd = MathHelper.applyGCD(serverYaw, serverPitch, prevServerYaw, prevServerPitch);
            serverYaw = gcd[0];
            serverPitch = gcd[1];
    ...
    {
        final MotionEvent event = new MotionEvent(this.posX, this.getEntityBoundingBox().minY, this.posZ, this.onGround,
                EventTime.PRE);
        ...
        {
            double d0 = event.posX - this.lastReportedPosX;
            double d1 = event.posY - this.lastReportedPosY;
            double d2 = event.posZ - this.lastReportedPosZ;
            double d3 = (double)(serverYaw - this.lastReportedYaw);
            double d4 = (double)(serverPitch - this.lastReportedPitch);
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(event.posX, event.posY, event.posZ, serverYaw, serverPitch, event.onGround));
                }
                else if (flag2)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(event.posX, event.posY, event.posZ, event.onGround));
                }
                else if (flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(serverYaw, serverPitch, event.onGround));
                }
                else
                {
            ...
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, this.rotationYaw, this.rotationPitch, event.onGround));
                flag2 = false;
            }
            ...
            if (flag2)
            {
                this.lastReportedPosX = event.posX;
                this.lastReportedPosY = event.posY;
                this.lastReportedPosZ = event.posZ;
